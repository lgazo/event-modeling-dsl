...
async function checkNPM(cancellationToken) {
    const { stdout } = await exec('pnpm -v', {}, cancellationToken);
...
function getNpmDependencies(cwd) {
    return checkNPM()
        .then(() => exec('pnpm list --production --parseable --depth=99999 --loglevel=error', { cwd, maxBuffer: 5000 * 1024 }))
...
function getLatestVersion(name, cancellationToken) {
    return checkNPM(cancellationToken)
        .then(() => exec(`pnpm show ${name} version`, {}, cancellationToken))
        .then(parseStdout);
}
